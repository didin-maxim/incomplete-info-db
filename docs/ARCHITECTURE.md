...
Для общей модели заранее выбранных вопросов да/нет используется `binary_question_code`: конфиг задает `object_count`, `max_tests`, `objective: "identify_hidden_number"` и опциональные `modes`. Viewer не показывает готовый код при открытии карточки: пользователь выбирает подмножества для вопросов, затем запускает проверку уникальности двоичных ответов; примерная стратегия доступна только по отдельной кнопке.
...
Кураторские списки внешних материалов, когда ссылка полезна для целой темы,
фрагмента или кластера, но не должна становиться новой карточкой задачи.
Файл содержит `resource_lists[]`; каждый список задает:

- `id`, `title`, `scope`, `items`;
- `scope.fragment` или `scope.cluster_id`, `language`, `focus`;
- `items[].source_id` - ссылка на запись из `data/sources/sources.yaml`;
- `items[].kind`, `use_for`, `why_useful`;
- опционально `linked_problem_ids`, если материал особенно полезен рядом с
  конкретными карточками;
- для видео/лекций опционально `video.title`, `video.author_or_channel`,
  `video.topic`, `video.why_useful`.

`tools/validate.py` проверяет, что `source_id` и `linked_problem_ids`
существуют, а обязательные поля заполнены. Viewer пока не читает этот слой; это
редакторский реестр для следующих агентов и составителей.
