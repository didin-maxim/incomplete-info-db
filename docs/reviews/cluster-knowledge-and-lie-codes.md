...
Исключал задачи, где есть публичные ответы, но они служат кодовым сообщением без итерации знания: шляпные протоколы с четностью или остатками, приватная карточная коммуникация, обычные статические рыцари/лжецы.
...
- покрытие по Хэммингу и совершенный код для шляп Эберта;
