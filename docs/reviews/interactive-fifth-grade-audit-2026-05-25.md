...
Уточнение после повторного аудита: реально слишком простым подтвержден только `permutation-encodes-six-messages`. Остальные пункты ниже в основном попали сюда из-за UX-утечек, кнопок готового разбора или слишком раннего показа состояния, а не из-за слабой математики.

- `permutation-encodes-six-messages` - очень базовый кликер: идея `3! = 6` почти полностью решает задачу.

Не считать слишком простыми без дополнительной проверки:

- `fixed-questions-as-binary-code` - математика требует реального кодирования 8 объектов тремя битами; проблема в кнопке примера, которая слишком легко загружает решение.
- `three-coins-unknown-sign-two-weighings` - маленькая, но честная задача на различение 6 состояний и планирование ветвления.
- `moebius-2018-three-coins-knight-liar-genuine` - задача не пустая; проблема в раннем показе совместимых состояний и доказанных ответов.
- `counterfeit-stack-one-weighing` - идея числовой подписи реальная; проблема в том, что viewer выдает единственное оставшееся состояние до ответа.
- `mccme-2016-three-piles-one-genuine-pile` - небольшая, но честная сертификационная задача.
- `thirteen-coins-preassigned-identify-only-three-weighings` - математика не слишком простая; проблема в стартовой готовой таблице и разборе сигнатур.

Пограничные случаи:

- `knop-saladin-four-weighings-one-spare` - не слишком простой; править нужно ранний доступ к готовому плану и полному разбору.
- `prisoners-hats-parity-line` - не слишком простой; править нужно раннее раскрытие идеи четности, демо и проверку всех раскладок.
