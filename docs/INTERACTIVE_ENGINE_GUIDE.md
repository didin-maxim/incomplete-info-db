...
| `single_counterfeit_unknown_direction` | Одна фальшивая монета, знак неизвестен. Цель может требовать монету и знак или только монету. | `counterfeit-12-coins-3-weighings`, `thirteen-coins-identify-only-three-weighings`, `thirteen-coins-known-genuine-three-weighings` |
...
| `constrained_light_counterfeit_sets` | Допустимые множества фальшивых заданы явно или геометрическим ограничением. | задачи Мёбиуса про круг/линию легких монет |
...
| `binary_cards_number_trick` | Число кодируется двоичными карточками признаков. | `calendar-card-binary-trick`, `fixed-questions-as-binary-code` |
...
- Заранее назначенные вопросы/карточки: `binary_cards_number_trick`, `ternary_question_code`, `subset_signature_protocol`, `balanced_subset_question_code`.
