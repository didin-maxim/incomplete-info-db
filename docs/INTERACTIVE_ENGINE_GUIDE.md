...
- `interactive_strength: strong` - пользователь решает содержательную часть задачи, а viewer проверяет скрытые случаи;
- `interactive_strength: weak` - действие есть, но интерфейс заметно ведет пользователя или проверяет только малую часть идеи;
